...
export const MainDashboardScreen: React.FC<MainDashboardScreenProps> = ({
  ...
  const totalExpectedRent = filteredTenants.reduce((sum, t) => sum + t.monthlyRent, 0);
  ...
  const handleSendSingleReminder = (tItem: TenantEntity) => {
    sendTenantReminder(
      tItem.name,
      tItem.phone,
      tItem.arrears,
      ...
      tItem.unitName,
      () => {
        setReminderToast(`SMS payment reminder dispatched to ${tItem.name} (${tItem.phone}).`);
  ...
  return (
    <div className="space-y-6 pb-20 max-w-7xl mx-auto px-4 sm:px-6 pt-4">
      ...
      {/* Property Filter Chips & Quick Add */}
        ...
        {currentRole === 'LANDLORD' && (
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className="px-3 py-1.5 bg-[#F5F8F6] hover:bg-[#E2F8EF] border border-[#DFE8E3] text-[#17231E] rounded-xl text-xs font-bold transition-all flex items-center gap-1 cursor-pointer"
            >
              <Plus className="w-3.5 h-3.5 text-[#0AB77F]" />
              <span className="hidden sm:inline">Add Estate</span>
            </button>
            <button
              onClick={() => setShowAddTenantModal(true)}
              className="px-3 py-1.5 bg-[#0AB77F] hover:bg-[#07885E] text-white rounded-xl text-xs font-bold transition-all flex items-center gap-1 cursor-pointer"
            >
      ...
      {/* ZERO-DATA INTELLIGENT EMPTY STATE IF NO PROPERTIES EXIST */}
      {properties.length === 0 ? (
        <EmptyState
          icon={Building}
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="bg-[#101915] rounded-3xl p-6 text-white shadow-xl border border-white/5 flex flex-col justify-between space-y-5">
              <div className="flex items-center justify-between">
                ...
                <span className="px-2.5 py-1 rounded-lg bg-[#0AB77F] text-white text-[11px] font-black">
                  {filteredPayments.length} {t.receipts}
                </span>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="text-[#9FB2A9] font-semibold">
                    {t.occupancy}: {occupancyRate}% ({tenants.length}/{totalUnits} Units)
                  </span>
                </div>
              </div>
            ...
            <div
              className={`rounded-3xl p-6 shadow-md border flex flex-col justify-between space-y-5 ${
                totalPendingArrears > 0 ? 'bg-white border-red-200' : 'bg-white border-[#DFE8E3]'
              }`}
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                ...
                >
                  {totalPendingArrears > 0 ? `⚠️ ${tenantsInArrears.length} Units Overdue` : '✅ All Clear'}
                </span>
              </div>
          ...
          {/* NET CASHFLOW & EXPENSE METRIC STRIP */}
          ...
          {tenantsInArrears.length > 0 && (
            <div className="bg-white rounded-3xl p-6 border border-[#DFE8E3] shadow-sm space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {tenantsInArrears.slice(0, 4).map((tItem) => (
                  <div
                    key={tItem.id}
                  ...
                  >
                    <div className="space-y-0.5">
                      ...
                      <div className="text-xs font-black text-[#D93838]">
                        Owed: {formatUgx(tItem.arrears)}
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <button
                        onClick={() => handleSendSingleReminder(tItem)}
                      ...
                      <button
                        onClick={() => {
                          setSelectedTenantForPayment(tItem);
                          setShowRecordPaymentModal(true);
                        }}
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { id: 'maintenance', icon: '🛠️', label: 'MARS Projects Uganda', desc: 'Repairs & Renovations' },
                { id: 'landlord_settings', icon: '👑', label: 'Landlord Settings', desc: 'Managers & Policies' },
          ...
          {/* RECENT VERIFIED PAYMENTS STREAM */}
          <div className="bg-white rounded-3xl p-6 border border-[#DFE8E3] shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              <div>
            ...
            {filteredPayments.length === 0 ? (
              <div className="text-center py-8 text-[#65766F] text-xs">
                No payment transactions recorded yet. Click "Record Payment" to log your first receipt.
              </div>
            ) : (
